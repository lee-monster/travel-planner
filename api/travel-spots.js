...
// Server-side rendered spot page for social media OG tags
async function renderSpotPage(req, res) {
  ...
  try {
    ...
    const e = escHtml;
    const ogTitle = e(name + ' — TravelKo');
    const ogDesc = e(description.substring(0, 200));
    ...
    const localeMap = {en:'en_US',ko:'ko_KR',id:'id_ID',mn:'mn_MN',ms:'ms_MY',vi:'vi_VN'};
...
${lat && lng ? `<meta property="place:location:latitude" content="${lat}"><meta property="place:location:longitude" content="${lng}">` : ''}
<link rel="canonical" href="${e(spotUrl)}">
<script type="application/ld+json">${JSON.stringify({"@context":"https://schema.org","@type":"TouristAttraction","name":name,"description":description.substring(0,300),"image":ogImage,"address":{"@type":"PostalAddress","addressCountry":"KR","streetAddress":address},...(lat&&lng?{"geo":{"@type":"GeoCoordinates","latitude":lat,"longitude":lng}}:{}),"isPartOf":{"@type":"WebSite","name":"TravelKo","url":"https://travel.koinfo.kr"}})}</script>
<style>*,*::before,*::after{box-sizing:border-box;margin:0;padding:0}:root{--p:#2563EB;--pd:#1D4ED8;--g50:#f9fafb;--g200:#e5e7eb;--g500:#6b7280;--g800:#1f2937;--g900:#111827;--r:8px}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:var(--g800);background:var(--g50);line-height:1.6;min-height:100vh;display:flex;flex-direction:column}a{color:var(--p);text-decoration:none}header{background:#fff;border-bottom:1px solid var(--g200);padding:12px 20px;display:flex;align-items:center;justify-content:space-between;max-width:720px;width:100%;margin:0 auto}.logo{display:flex;align-items:center;gap:8px;font-size:1.1rem;font-weight:700;color:var(--g900);text-decoration:none}.logo img{width:28px;height:28px;border-radius:4px}.badge{font-size:.72rem;font-weight:600;color:var(--p);background:#eff6ff;border:1px solid #bfdbfe;padding:3px 10px;border-radius:12px}main{flex:1;max-width:720px;width:100%;margin:0 auto;padding:24px 20px 40px}.spot-img{width:100%;max-height:400px;object-fit:cover;border-radius:var(--r);margin-bottom:16px}.spot-cat{display:inline-block;font-size:.78rem;font-weight:600;padding:3px 10px;border-radius:12px;background:#eff6ff;color:var(--p);margin-bottom:8px}h1{font-size:1.4rem;font-weight:700;color:var(--g900);margin-bottom:8px}.spot-meta{font-size:.85rem;color:var(--g500);margin-bottom:16px}.spot-desc{font-size:.95rem;line-height:1.7;margin-bottom:20px}.spot-addr{font-size:.88rem;color:var(--g500);margin-bottom:24px}.cta{text-align:center;margin-top:20px;padding:24px 16px;background:#fff;border:1px solid var(--g200);border-radius:var(--r)}.cta p{color:var(--g500);font-size:.9rem;margin-bottom:14px}.cta-btn{display:inline-block;background:var(--p);color:#fff;font-weight:600;font-size:.92rem;padding:10px 28px;border-radius:var(--r);text-decoration:none}footer{text-align:center;padding:20px;font-size:.78rem;color:var(--g500);border-top:1px solid var(--g200);max-width:720px;width:100%;margin:0 auto}@media(max-width:480px){main{padding:16px 14px 32px}h1{font-size:1.2rem}.spot-img{max-height:260px}}</style>
</head>
<body>
<header><a href="/" class="logo"><img src="/images/main_bar.png" alt="TravelKo">TravelKo</a><span class="badge">${catEmoji} ${e(category)}</span></header>
<main>
${ogImage && ogImage !== 'https://travel.koinfo.kr/images/og-travel_2.png' ? `<img class="spot-img" src="${e(ogImage)}" alt="${e(name)}">` : ''}
<span class="spot-cat">${catEmoji} ${e(category)}</span>
<h1>${e(name)}</h1>
<div class="spot-meta">${e(region)}${address ? ' · ' + e(address) : ''}</div>
<div class="spot-desc">${e(description)}</div>
...
<div class="cta"><p>Explore more spots on TravelKo</p><a href="${e(appUrl)}" class="cta-btn">Open in TravelKo</a></div>
</main>
...
module.exports = async function handler(req, res) {
  ...
  // Serve spot page HTML if render=page
  if (req.query.render === 'page') {
    return renderSpotPage(req, res);
  }
